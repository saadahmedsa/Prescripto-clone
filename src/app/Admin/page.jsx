...
import api from "../../helper/api"; // <-- your axios helper
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        // Fetch doctors
        const { data: doctorsData } = await api.get("/all");
        // Fetch patients
        const { data: patientsData } = await api.get("/users");
        // Fetch appointments
        const { data: appointmentsData } = await api.get("/appointments/today");
    ...
    <div className="p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        <div className="p-6 bg-white rounded-xl shadow-md">
          ...
          <p className="text-3xl font-bold text-purple-600 mt-2">{stats.patients}</p>
        </div>
              ...
              >
                <div className="flex justify-between items-center gap-1">
                  ...
                  <p className="flex items-center gap-2 text-sm text-gray-600">
                    <Stethoscope size={16} className="text-gray-400" /> {appt.doctor?.name}
                  </p>
                  <p className="flex items-center gap-2 text-sm text-gray-600">
                    <CalendarClock size={16} className="text-gray-400" /> {appt.timeSlot}
                  </p>
